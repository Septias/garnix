...
|---|---|---|
| the judgement itself | ✔ `Infer` / `InferRec` (lean/Infer.lean) | — a RELATION, not a function: a function would owe unification's termination, `A-let`'s fixpoint and the `↝*` closure, none of which exist. Non-vacuity is pinned by `selEx_infers` |
| **`⟦S⟧` as a context** | ✔ `Sol.toCtx`, `Sol.lookup_toCtx_iff` (RowUnify/State.lean), `SolverState.applyCtx` (Infer.lean) | STALE ROW — State.lean closed this. The bridge is conditioned on `Sol.WF`, so it waits on `UnifyWF`; `⟦S⟧` in the A-rules is currently `Sol.toSubst` (one step), which agrees with the closure on a well-formed state |
...
| **name supply in the inference rules** | ◐ threaded (`SolverState.supply`, `draw`) | FOUND WHILE WRITING THE RULES: `unifyTyM`/`unifyRowM` start from a LOCAL supply (`lenBound … + 1`, `localSupply`), which can hand back a supply BEHIND the state's and let a later `draw` re-issue a live name. `SolveTy`/`SolveRow` therefore call `unifyTyF`/`unifySpineMF` with `S.supply` threaded in and out. Still missing: the monotonicity THEOREM, and Γ-freshness |
...
| **rules for non-success verdicts** | ✔ `Infer.appDeg`, `Infer.selDeg` + `SolveTyDegrades` | a clash has NO rule — that IS the hard error, and it is sound by `unifyM_clash_no_unifier`. Totality is still not PROVED, but it is now stateable |
| parking a fresh constraint | ✔ `Wakes.park` | the blocker is computed by exhibiting a `LookupBlocked` witness, which is exactly the missing `K-park`. `LookupBlocked` also supplies the paper's `? on α`, which `Lookup` does not record |
...
| soundness of degradation | ✘ | NOT what this row used to say. "Replacing a position by ★ preserves declarative typeability" is about a term that already types; when `A-app-degrade` fires the application types *nowhere* — `qApp` wants a literal arrow, `qEq` only moves along ≈, and ≈ relates ★ to nothing but itself, so `qUnk` cannot manufacture one. What is missing is a **declarative rule for application at ★** (the ★-elimination the failure policy assumes), or a proof that the rule is unreachable. `A-sel-degrade` plausibly *is* unreachable: `r` is drawn fresh immediately before `τ ≐ {r}`, so that equation can only clash or succeed. `A-app-degrade` is not — its equation descends into an arbitrary row problem. Found 2026-09-16 while proving `InferSound` case by case (lean/InferSound.lean) |
