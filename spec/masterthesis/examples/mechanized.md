...
reduct is untypeable. 

Better:  g: { a = g ({}.l) | b = g ({l = c′}.l) }
...
- Why do we even admit the type {ε} → ★ ??
...
---------------------------------------
Any plain scheme covering both typings above must have a QUANTIFIED VARIABLE
in its result position (it instantiates to both {ε} and ★, which are rigid constructors).
But then re-pointing that variable at {ε} inside the ⊥-instance's substitution
...
∀β δ. ⟨β.l ↓ δ⟩ ⇒ {β} → δ
---------------------------
The result position δ stays WRITABLE per instance and discharge
...
and the mixed instance is now correctly EXCLUDED: θ must send
...
[Δ₂]ρ₁ = [Δ₁]ρ₁. We need no side condition at all: rows mod ≈ form a **trace monoid**,
 which is left- AND right-cancellative, so U-var-refl right-cancels the shared α
on the nose and the l₁-projection then clashes. **The algebra replaces the side condition**
— this is the single biggest simplification the ≈-characterization buys.
