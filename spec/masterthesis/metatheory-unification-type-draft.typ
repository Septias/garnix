// DRAFT — a new subsection for `= Metatheory <metatheory>` in thesis.typ.
// Slots in AFTER `== What unification proves <unification-metatheory>`
// (i.e. after line 881, immediately before `= Extensions to the minimal
// Calculus <sec-extensions>`). It closes the unification metatheory by saying
// why the #u_stuck verdict of the preceding subsection cannot be engineered away.
//
// Alternative placement: directly after `== Row equivalence is a trace monoid
// <trace-monoid>`, if you would rather present it as a property of the theory
// than as a limit on the algorithm. The text below assumes the first placement
// (it refers back to @unification-metatheory and @trace-monoid).
//
// Fragment: not compilable on its own (uses #u_stuck from thesis.typ, refs
// @trace-monoid, @unification-metatheory, @principality, @our-position, and the
// bibliography). Two bib entries are needed — see the block at the bottom.

== What unification cannot prove <unification-type>

The preceding subsection leaves one verdict unaccounted for. #u_stuck is reported when every move is dead but nothing is provably wrong, and read naively that is an admission: a better algorithm, with more arms, would reach an answer. This subsection shows that it is not. The obstruction is a property of row equivalence itself, and no algorithm that returns a solution — or finitely many — can avoid it.
...
- *Every unifier makes α variable-free.* If θ unifies the shift problem then $"vars"(θα) = []$ (`shift_unifier_varFree`). The equation admits no symbolic answer whatsoever. The mechanism is the trace-monoid presentation of @trace-monoid read backwards: the trailing field sits at segment index $|"vars"(θα)|$ on the left and at index 0 on the right, and pointwise agreement of the $l$-projections walks the list down until $|"vars"(θα)| = 0$ (`shift_proj_forces_zero`).
