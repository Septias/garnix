...
A with statement binds _weakly_, meaning it will not override identifiers that were introduced by enclosing functions or let-bindings. For example `let a = 2; in (with {a = 3;}; a)` evaluate to 2 instead of 3, even though the binding `a = 3;` is evaluated after `a = 2;`. The same behaviour arises with function parameters `a: (with {a = 2;}; a) → a`, inherited bindings, and records. For example, the expression `rec {a = 3; b = (with {a = 4;}; a);}` will evaluate to `{a = 3; b = 3;}`. When stacking with-bindings, shadowing behaves as expected: `with {a = 1;}; with {a = 2;}; a → 2`.
...
In the following section, we survey relevant type-system features and assess their applicability to Nix.

In practice, most users engage with nix through the module systems of nixos and home-manager to configure their operating system or user environment. Within this context, autocompletion for option values that consist of a type, default-value, example and description is most valuable. Although both module systems provide online services #footnote(link("https://home-manager-options.extranix.com/")) #footnote(link("https://search.nixos.org/options")) for this information, there exists no satisfactory solution that works in IDEs #footnote("Integrated Development Environment").

The module system is a part of the nix standard library and utilizes the languages core features, such that full option inference is a feature that builds upon nix-language type inference. In principle, Nix type inference could determine precise types for option values and improve completion, but it would not provide the accompanying descriptions and examples. A natural direction for future work is to quantify the benefits of Nix type inference for option autocompletion and to evaluate whether leveraging existing web‑portal metadata for completion is preferable.

Developers also use Nix as a programming language in the standard library and across the packaging ecosystem. Although this group may be smaller, the benefits of reliable type inference are substantial and have been argued to be necessary to make the language complete @nix-ts-issue. A well‑designed type system would further lower the barrier to entry for new users.

Our goal is to derive a type system for the core language, rather than a DSL that targets only the module system. The following section develops this direction.


== Wanted Properties
The nixpkgs repository, comprising roughly 40,000 files and 120,000 packages, is the largest and most up‑to‑date package collection in existence; it receives about 80 PRs#footnote("Pull Requests") per day, has nearly 100,000 commits, and maintains thousands of open issues and PRs. The sheer size of the ecosystem has two immediate consequences: First of all, type inference needs to take into account that all code (libraries, packages, built-tools, nixos, modulesystem, etc.) root in _a single file_ #footnote([All of nix' features root in a single file at #link("https://github.com/NixOS/nixpkgs/blob/master/flake.nix") or #link("https://github.com/NixOS/nixpkgs/blob/master/default.nix"), depending on whether one uses a flake based system or not.]) so an inference algorithm that tries to fully evaluate the whole tree would need to evaluate all 120.000 packages. Since no typeinference algorithm exists yet, we can only guess, but it seems like a _lazy_ type inference algorithm @lazy_inf might be needed to handle such a huge syntax tree.
