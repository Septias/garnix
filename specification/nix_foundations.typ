...
The let-binding of the second example is very similar to a recursive record in that it allows for _multiple_, possibly mutal-referencial bindings, `let a = b; b = 2; in b`. It does not need a `rec` keyword but is equally not permitted to create mutual recursive non-constructive recursion like `let a = b; b = 2; in b`, because, again, there is no lazy constructor that remedies infinite computation. Their similarity is best shown by the let-rec-in-binding `let {a = 2; b = 3;} in t` that encloses the semicolon-seperated key-value bindings of the usual let-binding in braces. It is trivial to see that both let-forms can be rewritten to one another by removing or adding the enclosing braces, rendering the latter notation valid, but obsolete.
...
The import-statement can occur anywhere in the syntax-tree, including recursive functions, struct-fields, arrays and literally every other construct. Since the import statement will "fill in" the files evaluated content literally, it is possible to follow the statement up with an arbitrary value. A statement like `import ./modules.nix {system = "nixos_x86"}` will then first evalute the file `modules.nix` and interprete the following term as an argument to a function, resulting in a function application. `import ./modules.nix {system = "nixos_x86";} -> f {system = ""}`. The integrity of functions in files is not checked statically, but it is a frequently used pattern to seperate code into different files. To illustrate the point, see @module_example for a full example.
...
`__overrides__` is a record-field, that is recognized by the language evaluator and can be used to override fields of the record. `{ a = 1; __overrides = { b = 2; }; -> {a = 1; b = 2;}`. The other field special to nix is the `__functor__` field that can be used to turn a record into a function. `({ __functor = self: x: self.foo && x; foo = false; } // { foo = true; }) true -> true`. Lastly, the `__toString` dunder allowes one to define a string representation for a record.
...
= Syntax <syn>
#set raw(lang: none)
$overline(E)^{i ∈ 𝓘}$ denotes a repitition of a syntax construct indexed by $i ∈ 𝓘$. The index $i ∈ 𝓘$ is omitted if obvious.
...
Over the years, logical _type connectives_ like union $τ ∨ τ$, intersection $τ ∧ τ$ and negation $¬τ$ have found their way into many mainstream languages @flow @typescript @typed_racket @mlstruct, proving their usefullness by giving an intuitive method to combine otherwise unrelated types. For example, a function that uses a conditional `x: y: z: if x then y else z` is a function that that returns either y or z based on whether x is true or false. This function can be typed at $bool -> α -> β -> (α ∨ β)$, intersecting the possible return types. This is a great improvement to previous ml-like systems that would have to _unify_ α and β in this situation which is already not possible for integers and strings @algebraic_subtyping. This is because the intersection based type respects the variable _flow_, only merging α and β because they flow together in the output.

*overloading* Using intersection types, one can define functions that have different types. For example the function `if isBool(x) then !x else x + 1` is a function that either inverts a bool or adds one to an integer. We would like to describe the argument x with a type-variable α, but from the function body it is clear, that this function is only well-behaved on integers and bools. This function can be given two types. The first one $(bool ∨ int) -> (bool ∨ int)$ states that this function can be given an argument of either bool or int and will return either an int or bool. But using intersection types, the functiontype can be refined to the more specific type $(int -> int) ∨ (bool -> bool)$, stating that if the function is called with an integer, it will also return one (instead of the union $int ∨ bool$).

In languages with type intersection types, this function can be given the type (`α -> (bool ∨ int)`) denoting that given any argument, it will either return a bool or an integer. If negation types are also admitted, then the function type can be refined to `(bool -> bool) ∨ (α ∧ ¬bool -> α)` which is the most general type for this function. The nix addition operator can be used on strings and paths likewise. It is such possible to write expressions like `/home/ + "john" -> /home/john` that will return a _path_ and `"/home/" + "john"` which will return a _string_. The most general type is thus `(str -> (str ∨ path) -> string) ∧ (path -> (str ∨ path) -> path)`, needing intersection types.


*pattern-matching and flow typing* _Flow typing_ and the related _occurrence typing_ are techniques used in _gradual type systems_ that narrow a type based on its usage. For example, the function `if isBool(x) then !x else x + 1` checks the runtime value of x to be of type bool. After this conditional check, one can obviously type the positive branch under the assumption, that x is of type bool @typescript. A similar technique can be used to refine the consecutive branches of pattern matching statements.
In an example statement `match x with bool(x) -> .. | rec(x) -> .. | _ -> x` one matches the single branches in order. The normal branches behave like conditionals, forcing a type on the variable, the interesting case is the default-case. This one can be typed under the assumption, that it is not of type bool or record.
...
*subtyping* The last but maybe most important feature of
