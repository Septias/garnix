...
The Nix programming language is used in over 40,000 source files, showing its practical significance and widespread adoption. Despite this prominence, the language has not yet received a static type system such that practitioners need to wait for the evaluator to return a result; A tendious and time-intensive process. Independent efforts by S. Klähn and by Breokhoff et al. have begun to address the shortcomings of the theoretical treatment of the language by providing a simple type inference approach and operational semantics. The present work extends this line of research by developing an operational semantics that more comprehensively reflects the language’s behavior, and also systematically examines potential approaches to type inference in this setting.

= Origin of the Nix Language <intro>
The nix package manager distinguishes itself from other package managers because it is founded on a built-in domain-specific programming language. This eponymous language, Nix, is a  major source of the ecosystem’s steep learning curve but is also the foundation of its core properties — most notably its _reproducibility_ guarantees.
...
#[
  ...
  #figure(
    ...
    table(
      ...
      [attrNames `set`             ], $ recordType -> [ overline(l_i)^i ] $,
...
@builtin_excerpt shows an excerpt of the Nix builtins, the full list can be found in @all-builtins. The record‑ and list‑oriented builtins extend these constructs in predictable ways and motivate the need to closely model record and array values to form precise return types for these functions. For example, `attrNames` motivates the need for first-class labels and both the `attrValues` and `catAttrs` functions use type connectives in their return type, a feature that is further discussed in @connectives.

The deserializers `fromJson` and `fromToml` pose challenges for static typing: given an arbitrary string, they may produce any Nix value. Absent a faithful model of the decoding process, the most precise static approximation of the return type is the unknown type $star.op$ that directly motivates the need for an _occurrence typing_ or _soft_ typing system.

The `is*` predicates can be used to inspect the type of expressions at runtime. This feature enables refinement of types in conditionals and motivates the addition of _occurrence typing_. An example expression is `if isBool(x) then !x else false`. The positive branch of this conditional can be typed under the assumption, that x is a bool because it is only reachable under this condition. Furthermore, it is possible to write functions that act according to a supplied type. This motivates the addition of _overloaded_ functions to the typesystem.
...
```
writes an arbitrary string to a file and immediately reads and executes the resulting file. This feature is equally strong to the generic `eval` struct to Javascript of Python.
...
Let-expressions can define multiple bindings $a_1 = t_1; … ; a_n = t_n$, possibly referencing each other in a recursively. Both let-statements and records allow inherit-statements to be placed between ordinary field declarations. Inherit statements take a known label for a value and _reintroduce_ the label as "label = value;" to the record or let expression. They can also take a root path $p$ which is prefixed to all following labels. This way, a deep record can be referenced from which all values are taken. For example, the statement `inherit (world.objects.players) robert anders;` will desugar to `robert = world.objects.players.robert; anders = world.objects.players.anders;` in the surrounding record or let-expression.
...
We follow the semantics of broekhoff and krebbers @verified and assume proper operational semantics for the primitive Algebraic, Logic, Pipe and Comparison operators and give explicit transition rules for Records and Array operators. The evaluation order is call-by-name which is operationally equivalent to lazy evaluation but less performant in interpreters. The binding-power kind $k$ is used to annotate the variable binding power in substitutions to properly handle the weak let-expression binding power. $rec$ and $nonrec$ are used to tag the recursiveness of of record-fields. A recursive record will thus be rewritten from `rec {a = 2; b = 3;}` to `{ rec a = 2; rec b = 2}`. Recursive and non-recursive fields are needed to faithfully desugar inherit statements. A statement `rec {inherit x;}` would otherwise be rewritten to `{x = x;}`, forming a cyclic definition which is not faithful to the evaluator that raises an error because x is undefined when not provided by the surrounding expression. Furthermore R-Attr-Rec will use the rec tags to unfold a record with recursive fields to a non-recursive record using the auxiliary unfold function. This function unrolls the recursive bindings of a record and internally uses the indirects function. The indirects function is used to create a substitution of abs-strength from record bindings.

The rule R-Final is used in conjunction with _deferred substitutions_ @verified. Deferred substitutions reify substitutions on variables until variables are needed and is motivated by the following example. The expression `with g {}; a + b` is only _closed_ if the function application returns a record that has the a an b variables as fields. An _outer_ or previous with-binding should thus not overwrite the a and b bindings but needs to delay the substitutions until `with g {}` is evaluated. The rule R-Final takes the resulting annotated variable and returns its prevalent definition. The definition of _deferred substitutions_ is given in @substitution and the first case of the variable case gives precedence to bindings with abs-strength, the second handles the first binding and overwriting with-bindings. The last case skips variables that are not in the parallel substitution. A variable without any annotations is annotated with $"None"$ and a variable with a binding as $"Some" k t$, keeping track of the binding power $k$ and the term $t$ that should be substituted.

The rule `R-match` to match a record argument against a pattern uses the auxiliary `p ~ r ~ α` judgement that is  read as "Pattern p is matched with record r, giving a substitution α". The rules defined in @matching account for open and closed patterns as well as recursiveness in function patterns by using the same rec/nonrec tagging mechanism and deferred substitutions on the function body.

The conditional and record reduction rules are trivial. R-Record-Concat uses the right-biased union operator $union.arrow$ to denote that all bindings of the second argument take precedence over the first argument. The import rule takes a file-path and evaluates its content. The final expression is substituted as-is into the surrounding expression. Finally, the R-Dyn rule remove the surrounding braces and dollar sign of dynamic lookups and bindings as soon is the inner value is a string.
...
The development of function patterns, with-typing and occurrence typing seems to be perpendicular to the aforementioned typesystem research line and can be developed in separation. Since an unknown type is essential in face of language impurities and absence of type annotations, research along _soft typing systems_ seems interesting and could offer techniques to handle impurities correctly but lacks subtyping, type connectives and an unknown type.

The reduction semantic is based upon the works of Broekhoff and Krebbers @verified but extends their work with more syntax features, namely dynamic binding, dynamic lookups and string interpolation besides some smaller changes. We disregard their closer model of terminating and non-terminating computation to give a simpler presentation of reduction rules. Their work also examines the dynamic eval construct of javascript and other dynamic languages but does not apply these developments because Nix does not provide a dynamic eval primitive. We have seen in @builtins that it is possible to mimic the eval construct behaviour using the Nix builtins so adapting their semantic for this constructed version of eval remains an open task.
