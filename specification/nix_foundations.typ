...
Nix needs to be a _lazy language_ because of the costly environment where a single action – building a package – can be very expensive. In such an environment, packages must only be realised when they are needed. In a lazy language, values of function application are substituted as-is without further reduction i.e computation. Nix' lazy record fields then form the essential ingredient to not build packages if not _actually needed_ and saving valuable computation resources.
...
The record‑ and list‑oriented builtins extend these constructs in predictable ways but the deserializers `fromJson` and `fromToml` pose challenges for static typing: given an arbitrary string, they may produce any Nix value. Absent a faithful model of the decoding process, the most precise static approximation is the unknown type $star.op$. The `is*` predicates support runtime type inspection, enabling ad‑hoc overloading and occurrence typing. Finally, `abort` interacts with control flow by terminating evaluation unconditionally; it therefore has return type ⊥.
...
@syntax-table shows the syntax of nix with the usual literals of expressive languages as well as a multi-line string and paths. The syntax follows the official regex formulas of the informal nix specification @nix-language-2-28. Records follow a standard notation with multiple `key = value;` assignments. In addition, records can be marked _recursive_ with the `rec` keyword and are non-recursive otherwise. Arrays behave similarly and can be  concatenated with the only unintuitive nix-specific distinction that a space is used as separator. Both datatypes are generally _immutable_, but there are concat operations that can be used to create new, bigger datatypes. Furthermore, records can be accessed using labels, strings and a dynamically computed expression. Label access is standard and access by string a common technique in real world programming languages to allow more characters in records keys. Records have a check operation $t space ? ρ$ that returns a boolean as a result and the or-operator to specify a default value in case the previous check turned out to be negative.
...
Nix is a dynamically typed, lazy, and purely functional language. Its core features include extensible attribute sets (records), pattern-based functions with parameter destructuring, first-class labels, overloaded operators, and a reduction semantic with two levels of binding power. In addition, the language provides 78 built-in functions that operate on attribute sets and lists, access the execution environment, and support limited forms of type reflection.
...
The module system is a part of the nix standard library and utilizes the languages core features, such that full option inference is a feature that builds upon nix-language type inference. It is in principle possible to determine types for option values and improve completion, but whether one could gain the precision needed to faithfully deduce informative types for options is not immediately clear. Also, type inference would not provide the accompanying descriptions and examples. It is thus a natural direction for future work to quantify the benefits of Nix type inference for option autocompletion and to evaluate whether leveraging existing web‑portal metadata for completion is preferable to an inference-based approach.
...
== Wanted Properties
Idea: code examples for wanted properties?


The nixpkgs repository is, with its 40.000 files and 120.000 packages, the biggest and most up-to-date package repository in existence, receiving approx. 80 PRs#footnote("Pull Requests") a day and having close to 100.000 commits and multiple thousand open issues and PRs. This scale has two immediate implications for type‑inference. First, the evaluation root is effectively a single entry point #footnote([Most features of Nixpkgs are reachable from #link("https://github.com/NixOS/nixpkgs/blob/master/flake.nix") or #link("https://github.com/NixOS/nixpkgs/blob/master/default.nix"), depending on whether a flake‑based workflow is used.]), so an inference algorithm that tries to fully evaluate the whole tree would need to evaluate all 120.000 packages. Since no typeinference algorithm exists yet, we can only guess, but it seems like a _lazy_ type inference algorithm @lazy_inf might be needed to handle such a huge syntax tree.

todo: next two sections sound weird. why do I need gradual type system?
Second, changes in an ecosystem of this magnitude are expensive and slow such that the ecosystem can not switch to static typing "at once". A gradual typing discipline @gradual_siek @gradual_tobin, which interposes an unknown type $star.op$ at the boundary between dynamically and statically typed fragments, enables incremental adoption while preserving existing workflows.

Type annotations may be introduced in two primary ways: (i) as non‑intrusive comments, or (ii) as first‑class syntax within the language. The comment‑based approach, proven effective in the JavaScript/TypeScript ecosystem @typescript, avoids changes to the core grammar but competes with the conventional use of comments. In contrast, integrating annotations into the syntax would offer clearer tooling hooks at the cost of coordinated language evolution and a potentially substantial repository‑wide migration. A third alternative is full type inference in the Hindley–Milner tradition, which requires no annotations while recovering principal types for expressions. From a usability perspective, this imposes the least friction on existing code. Regardless of surface design, the inference procedure must be efficient at Nixpkgs scale; in particular, global backtracking is unacceptable, as it would render analysis prohibitively slow.

Besides the properties that are required due to the language' environment, a few properties are directly founded by the features of the language. First and foremost are _recursive types_, _first class labels_ and _record concatenation_ because of its strong record calculus. As surveyed in @records, these capabilities have each been treated in the literature but were only recently combined @extensible_tabular @extensible_rec_funcs @extensible_data_adhoc and they have not yet been consolidated in mainstream languages. In addition, Nix requires _parametric polymorphism_ to capture generalization across let‑bindings, and _ad‑hoc polymorphism_ to model overloaded operators and user‑defined dispatch via type inspection.
