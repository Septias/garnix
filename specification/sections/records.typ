...
#let export = [
  ...
  Nix records are immutable: updates are realized by construction rather than by in-place mutation, and there are no primitive add or delete operations. To regain expressiveness, the language provides _free record extension_ via the right-biased concatenation operator (`a // b`). Typing this operator is subtle: safe use requires the ability to express both field extension and field absence (restriction) in the type system. Nix further admits first-class labels—attribute names computed by expressions—which complicates static tracking of present and accessed fields, especially under polymorphism. Finally, attribute sets are recursive, so an inference procedure must detect and account for reference cycles during type inference.
  ...
  Subtyping is a pervasive form of polymorphism that organizes types into a refinement hierarchy: any value of a subtype may be used wherever a supertype is expected. While the substitutability intuition is straightforward for ground types, its interaction with higher‑order functions, variance, and ? is subtle. Designing subtyping relations that are both expressive and tractable has therefore been a central challenge in type theory and programming‑language design.
  ...
  Following the seminal work on semantic subtpyping, Castagna has shown how to handle _records_ @poly_records @typing_records_etc, _occurrence typing_ @revisiting_occurrence @on_occurrence, _gradual typing_ @gradual_perspective @gradual_elixir and _first class labels_ @typing_records_etc, showing the expressiveness of the system. But this expressiveness comes at a cost, the cost of complexity. The first calculi surrounding semantic subtyping relied heavily on backtracking and were unreasonably slow for real world applications. Improvements have been made, using, for example, boolean formula abstractions to compute efficient solutions (TODOD: citation needed), but many techniques used to make CDUCE @CDUCE performant are not even documented (from personal corespondance with Stefan Wehr). Also, semantic subtyping relies on heavy theory to compute subtypes in presence of type connectives, similar to the work of Pearreaux @simplesub @mlstruct @invalml @superF which will be discussed later.
