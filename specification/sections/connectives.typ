...
#let export = [
  ...
  Intersection types support ad‑hoc overloading by ascribing multiple, behavior‑specific function types simultaneously. Consider `x: if isBool(x) then !x else x + 1`, which either negates a boolean or increments an integer. A coarse typing that only uses type unions is $(bool ∨ int) -> (bool ∨ int)$ but this type cane be refined using intersections. The type $(int -> int) ∧ (bool -> bool)$ states that integer inputs yield integers and boolean inputs yield booleans and can not be refined more.
