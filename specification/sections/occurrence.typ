...
#let export = [
  ...
  Gradual type systems @gradual_siek @gradual_tobin try to strike the balance between static type inference and fully dynamic languages by allowing both to coexist. These typesystems accomodate two type systems in their torso, one annotated with static guarantees and a dynamic one. The boundary between the two is formed by an unknown type $star.op$ that is used in the dynamic part and casts that lift unknown to a concrete type. There exist quite a bit of research around the topic @gradual_siek @gradual_tobin @cantblamethis @agt @gradual_extensible_rows @consistent-subtyping @blame_for_all but most of the type systems come with type annotations, a feature we would like to dodge in this work.

  A more promising approach is thus _flow typing_ and the more rigid _occurrence typing_ @revisiting_occurrence  @on_occurence that narrow types based on their usage. For example, the function `if isBool(x) then !x else x + 1` checks the runtime value of x to be of type bool. After this conditional check, one can obviously type the positive branch under the assumption, that x is of type bool. Also, a negative type is needed to type the else-branch under the assumption, that x is not a bool. A similar technique can be used to refine the consecutive branches of pattern matching statements.
  ...
  Since typing can not be solved completely (there will always be safe programs that can not be proven save with any typesystem), we need a _gradual type system_ or _soft typesystem_ that accounts for incompleteness. We want to stress that while this might sound bad first, major languages like @typescript have been shown to be incomplete in regards to subtyping distributing intersections and unions but proven usefull still.
