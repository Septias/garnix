...
#let export = [
  == From Static vs. Dynamic to Gradual to Occurrence <occurrence>
  ...
  Flow typing @flow, and in particular the more disciplined occurrence typing @revisiting_occurrence @on_occurrence, offers a complementary approach that refines types along control‑flow and don't immediately need casts and annotations. Consider `if isBool(x) then !x else x + 1`: the predicate tests at runtime whether `x` is a boolean. In the then‑branch, `x` is treated as a bool; in the else‑branch, negative information that `x` is not a bool enables alternative typing. Similar refinements arise across pattern‑matching constructs.
