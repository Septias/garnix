...
#let export = [
  ...
  Since nix is lexically scoped, it would also be possible to track the opened record withs in a seperate context, lookup variables normally (as they are stronger binding anyways) and only if a variable is undefined, we check the contexts.
