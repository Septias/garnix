...
#let builtin_types = figure(
  ...
  table(
    ...
    [break `v`                   ], $ α -> α $,
    [builtins                    ],
    $ {"abort": τ → bot, "all": (α -> bool) -> [α] -> bool, .. } $,
    [catAttrs `attr list`      ],
    $ [oj(recordType)_j] -> [str] -> [or.big τ_(j i)] "where" l_(j i) ∈ [str] $,
    [ceil `double`             ], $ num -> int $,
    ...
    $ [{path: str, "prefix": str}] -> str -> str $,
    [floor `double`              ], $ num -> int $,
    [foldl' `op null list`       ], $ (α -> β -> α) -> α -> [β] -> α $,
    [fromJSON `e`                ], $ str -> {} $,
    [fromTOML `e`                ], $ str -> {} $,
    [functionArgs `f`            ], $ (openPat -> α) -> {oi(l_i\: bool)} $,
    [genList `generator length`  ], $ (int -> α) -> int -> [α] $,
    [genericClosure `attrset`    ], $ ? $,
    [getAttr `s set`             ], $ l -> recordType -> τ_i "where" l = l_i $,
    [getContext `s`              ], $ str -> {omitted} $,
    [i getEnv `s`                ], $str -> str -> {omitted}$,
    [getFlake `args`             ], $ {omitted} -> path $,
    ...
    $recordType -> {oj(m_j : τ_j)} -> {m_j: τ_j | m_j ∈ overline(l) }$,
    [isAttrs `e`                 ], $ τ -> bool $,
    [isBool `e`                  ], $ τ -> bool $,
    [isFloat `e`                 ], $ τ -> bool $,
    [isFunction `e`              ], $ τ -> bool $,
    ...
    [langVersion                 ], $ () -> str $,
    [length `e`                  ], $ [α] -> int $,
    [lessThan `e1 e2`            ], $ num -> num -> bool $,
    [listToAttrs `e`             ], $ [oi((l_i,τ_i))] -> record $,
    [map `f list`                ], $ (α -> β) -> [α] -> [β] $,
    [mapAttrs `f attrset`        ], $ (α -> β) -> recordType -> "?" $,
    [match `regex str`           ], $ str -> str -> [str] $,
    [mul `e1 e2`                 ], $ num -> num -> num $,
