...
During his phd. Eelco Dolstra developed the package manager `nix` @dolstra_phd that has two unique properties: _reproducibly_ and _purity_. Together, these properties allow for predictable builds on different machines, fearless package upgrades, overcoming the DDL-hell @nixos_long and easy rollbacks in case something goes wrong @nixos_long. Since its initial release in 2003, the ecosystem has seen continuous growth, surging especially in the last two years with people longing for a package manager that makes dependency management fearless and reliable. The greates distinction to other package managers and reason for its distinct features is the underling _nix language_ that was created in conjunction with the package manager, boasting features that are a perfect fit for the usecase.

The conceptual idea behind nix, the package manager, was already found two years earlier in @memory_to_software and is quite natural: Software projects often break when distributed because of dependencies that are either unspecified or if specified, lack the neccessary precision to rebuild a working environment. The solution proposed by the authors of @memory_to_software is to apply a memory management regimen to software where files represent locations in memory and references are pointers. To successfully ship packages (components) from one system to another, one has to build the _closure_ over all its dependencies. Tracking dependencies is hard, but the nix language mitigates at least some problems.


== A domain specific language
The nix language was designed to overcome a specific set of problems from package managers. It uses the path primitive to track dependencies, builtin functions to "do the heavy lifting" and implements properties like laziness and purity to securely build packages, but only if needed. Let's look over some of these properties in detail.

In a pure language, the output of a function is soley defined by its inputs. Since no interior mutability can change the functions output, it can be stored and _reused_ if the function is called with the same arguments again. In nix, every package is a built from pure derivation and stored to a global accessible _store_ and if the component is needed multiple times, it can be taken from the store instead of building it again. This boils down to @max_lazy.

In the nix language, a package can be created using the `derivation` function that is  executed by the _evaluator_,which creates an isolated environment and runs the build script defined in it.
